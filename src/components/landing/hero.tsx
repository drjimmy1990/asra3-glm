...
export function Hero() {
  ...
  const siteName = data?.settings?.site_name || 'asra3.com';
  const heroTitle = data?.settings?.hero_title || 'I Build SaaS Products & Automation That Scale';
  const heroSubtitle = data?.settings?.hero_subtitle || 'From idea to launch — I help startups and businesses ship production-ready SaaS applications and intelligent workflow automations that save hundreds of hours and drive real revenue growth.';

  const stats = [
    { value: data?.settings?.hero_stat_1_value || '50+', label: data?.settings?.hero_stat_1_label || 'Projects Delivered' },
    { value: data?.settings?.hero_stat_2_value || '98%', label: data?.settings?.hero_stat_2_label || 'Client Satisfaction' },
    { value: data?.settings?.hero_stat_3_value || '3x', label: data?.settings?.hero_stat_3_label || 'Faster Time-to-Market' },
    { value: data?.settings?.hero_stat_4_value || '$2M+', label: data?.settings?.hero_stat_4_label || 'Revenue Generated' },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-1/4 right-1/4 h-96 w-96 rounded-full bg-primary/5 blur-3xl animate-float" style={{ animationDelay: '2s' }} />
      <div className="absolute top-1/2 right-1/3 h-48 w-48 rounded-full bg-primary/8 blur-2xl animate-pulse-glow" style={{ animationDelay: '1s' }} />
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center">
        ...
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }} className="mx-auto max-w-4xl text-4xl font-extrabold tracking-tight sm:text-5xl md:text-6xl lg:text-7xl">
